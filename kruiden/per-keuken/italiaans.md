# Italiaans

Voorraad voor de italiaans keuken. Producten kunnen ook in andere keukenoverzichten voorkomen.

| Product | Type | Vorm | Alternatieve naam | Opmerking |
|---|---|---|---|---|
| Basilicum | kruid | gedroogd | — | — |
| Bella Italia | kruidenmengsel | mengsel | — | Mengsel met knoflook, peper, ui en chili. |
| Chilipeper | specerij | gemalen | — | — |
| Knoflookgranulaat | smaakmaker | granulaat | garlic granules | — |
| Laurierblad | kruid | heel | — | — |
| Majoraan | kruid | gedroogd | — | — |
| Oregano | kruid | gedroogd | — | — |
| Peterselie | kruid | gedroogd | — | — |
| Rozemarijn | kruid | gedroogd | — | — |
| Saffraan | specerij | draadjes | — | — |
| Salie | kruid | gedroogd | — | — |
| Venkelzaad | specerij | heel | — | — |
| Zwarte peper | specerij | heel | — | — |
